...
def update_index(topic, output_path, started_at, orch_provider, orch_model,
    ...
    Each row records date, topic, providers, question/search counts, mode,
    and a relative link to the report file.

    Args:
        topic:          Research topic string
        output_path:    Path to the saved report file
        started_at:     datetime when the run started
        orch_provider:  LLM provider used for orchestration
        orch_model:     Model name used for orchestration
        synth_provider: LLM provider used for synthesis
        synth_model:    Model name used for synthesis
        question_count: Number of questions researched
        search_count:   Total number of web searches performed
        short:          True if executive summary mode was used
    """
    os.makedirs("output", exist_ok=True)
    index_path = "output/index.md"

    # Create index with header row if it doesn't exist
